        ...
        if context.configuration and context.configuration.blocking:
            try:
                ...
                if isinstance(response, Task):
                    # For Task responses, use the task status to determine outcome
                    # Pass the last message from history if available
                    last_message = response.history[-1] if response.history else None
                    if response.status.state == TaskState.completed:
                        await task_updater.complete(last_message)
                    elif response.status.state == TaskState.failed:
                        await task_updater.failed(last_message)
                    elif response.status.state == TaskState.canceled:
                        await task_updater.cancel(last_message)
                    elif response.status.state == TaskState.rejected:
                        await task_updater.reject(last_message)
            ...
            try:
                ...
                async for event in self.agent.astream(context):
                    if isinstance(event, Task):
                        # For Task events, use the task status to determine outcome
                        last_message = event.history[-1] if event.history else None
